...
# @arg $2 string Optional format string (supports %Y-%m-%d, %Y/%m/%d, %m/%d/%Y, %Y-%m-%d %H:%M:%S)
# @exitcode 0 Valid date
# @exitcode 1 Invalid date
# @example
#   shlib::dt_is_valid "2024-01-01" && echo "valid"
#   shlib::dt_is_valid "01/15/2024" "%m/%d/%Y" && echo "valid"
...
shlib::dt_is_valid() {
    ...
    local pat_yyyy_mm_dd='^[0-9]{4}-(0[1-9]|1[0-2])-(0[1-9]|[12][0-9]|3[01])$'
    local pat_yyyy_mm_dd_hms='^[0-9]{4}-(0[1-9]|1[0-2])-(0[1-9]|[12][0-9]|3[01]) ([01][0-9]|2[0-3]):[0-5][0-9]:[0-5][0-9]$'
    local pat_yyyy_sl_mm_dd='^[0-9]{4}/(0[1-9]|1[0-2])/(0[1-9]|[12][0-9]|3[01])$'
    local pat_mm_dd_yyyy='^(0[1-9]|1[0-2])/(0[1-9]|[12][0-9]|3[01])/[0-9]{4}$'

    # If explicit format provided, validate against that format only
    if [[ -n "$format" ]]; then
        case "$format" in
            "%Y-%m-%d")
                [[ "$datestr" =~ $pat_yyyy_mm_dd ]] && return 0
                ;;
            "%Y-%m-%d %H:%M:%S")
                [[ "$datestr" =~ $pat_yyyy_mm_dd_hms ]] && return 0
                ;;
            "%Y/%m/%d")
                [[ "$datestr" =~ $pat_yyyy_sl_mm_dd ]] && return 0
                ;;
            "%m/%d/%Y")
                [[ "$datestr" =~ $pat_mm_dd_yyyy ]] && return 0
                ;;
        esac
        return 1
    fi

    # No format specified - try all known patterns
    [[ "$datestr" =~ $pat_yyyy_mm_dd ]] && return 0
    [[ "$datestr" =~ $pat_yyyy_mm_dd_hms ]] && return 0
    [[ "$datestr" =~ $pat_yyyy_sl_mm_dd ]] && return 0
    [[ "$datestr" =~ $pat_mm_dd_yyyy ]] && return 0

    # Fall back to system date command for other formats
    if date -d "$datestr" +%s &>/dev/null 2>&1; then
        return 0
    fi
    if date -j -f "%Y-%m-%d" "$datestr" +%s &>/dev/null 2>&1; then
        return 0
    fi
...
#   shlib::dt_to_unix "01/15/2024" "%m/%d/%Y"   # outputs: 1705276800
shlib::dt_to_unix() {
    local datestr="$1"
    local format="${2:-}"
    local normalized=""

    # Helper function to convert to Unix timestamp
    _dt_to_unix_convert() {
        local d="$1"
        # Try GNU date first
        ...
        if date -j -f "%Y-%m-%d %H:%M:%S" "$d" +%s 2>/dev/null; then
            return 0
        fi
        return 1
    }

    # If explicit format provided, parse with regex and normalize
    if [[ -n "$format" ]]; then
        case "$format" in
            "%Y-%m-%d")
                if [[ "$datestr" =~ ^([0-9]{4})-(0[1-9]|1[0-2])-(0[1-9]|[12][0-9]|3[01])$ ]]; then
                    normalized="$datestr"
                fi
                ;;
            "%Y-%m-%d %H:%M:%S")
                if [[ "$datestr" =~ ^([0-9]{4})-(0[1-9]|1[0-2])-(0[1-9]|[12][0-9]|3[01])\ ([01][0-9]|2[0-3]):([0-5][0-9]):([0-5][0-9])$ ]]; then
                    normalized="$datestr"
                fi
                ;;
            "%Y/%m/%d")
                ...
                ;;
            "%m/%d/%Y")
                if [[ "$datestr" =~ ^(0[1-9]|1[0-2])/(0[1-9]|[12][0-9]|3[01])/([0-9]{4})$ ]]; then
                    normalized="${BASH_REMATCH[3]}-${BASH_REMATCH[1]}-${BASH_REMATCH[2]}"
                fi
                ;;
        esac
        if [[ -n "$normalized" ]]; then
            _dt_to_unix_convert "$normalized"
            return $?
        fi
        # Format didn't match, fall through to auto-detection
    fi

    # Try to auto-detect and normalize
    # YYYY-MM-DD
    if [[ "$datestr" =~ ^([0-9]{4})-(0[1-9]|1[0-2])-(0[1-9]|[12][0-9]|3[01])$ ]]; then
        _dt_to_unix_convert "$datestr"
        return $?
    fi
    # YYYY-MM-DD HH:MM:SS
    if [[ "$datestr" =~ ^([0-9]{4})-(0[1-9]|1[0-2])-(0[1-9]|[12][0-9]|3[01])\ ([01][0-9]|2[0-3]):([0-5][0-9]):([0-5][0-9])$ ]]; then
    ...
    # Fall back to system date for other formats
